  ...
  internal static class EnsureCorrectOdinVersion {
    ...
    private static void Init() {
      ...
      if (TryGetOdinInspectorVersion(out var inspectorVersion)) {
        ...
        if (iVer.Major == vVer.Major && iVer.Minor == vVer.Minor &&
            iVer.Build == vVer.Build) // Ignore Revision
        {
          TryInstall();
        } else {
          ...
          if (TryGetOdinInstallPath(out var path)) {
            var versionMismatchFile
                = path + "Odin Validator/ignoreVersionMismatch.txt";
    ...
    private static void TryInstall() {
      if (TryGetOdinInstallPath(out var path)) {
        ...
        if (requireUpdate) {
        ...
        #if ODIN_INSPECTOR
        else if (EditorPrefs.GetBool("ODIN_VALIDATOR_SHOW_GETTING_STARTED",
                     false)) {
          ...
          EditorApplication.delayCall += () => {
            ...
            if (t != null) {
              var action
                  = ExpressionUtility.ParseAction<bool, bool>(
    ...
    private static bool TryGetOdinInstallPath(out string path) {
      var t = Type.GetType(
          "Sirenix.Utilities.SirenixAssetPaths, Sirenix.Utilities");
      ...
      var v = t.GetField("SirenixPluginPath",
